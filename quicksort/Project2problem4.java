...
private static int merge(int low, int middle, int high) {
	...
	public static void main(String[] args) {
		...
		// changing range and size as new instructions
		for (int currIntRange = 0; currIntRange < 3; currIntRange++) {
			if (currIntRange == 0) {
				range = 10000; // case 1
			} else if (currIntRange ==1){ 
				range = 100000; // case 2			
                        } else 
                                range = 1000000; // case 3

			//change the size as new instruction 
		...
		System.out.println("mergeSort() Average duration(" + avgDuration + ") Average comparisons(" + avgComparisons + ")");
		...
		for (int currIntRange = 0; currIntRange < 3; currIntRange++) {
			...
			for (int currIntSize = 0; currIntSize < 1; currIntSize++) {
				...
				for (int currTrial = 0; currTrial < numTrials; currTrial++) {
					...
					start = System.currentTimeMillis();
					// perform sort
					int comparisons = quicksort(0, size - 1, true, true);
					// get ending time
					finish = System.currentTimeMillis();
		...
		System.out.println("quicksort() Average duration(" + avgDuration + ") Average comparisons("+ avgComparisons + ")");
	}
}

// this shows both quicksort and merge sort's comparisons and durations
// we can see that quicksort is faster with less comparisons
// and as range = range*10, comparisons and durations increases (merge sort increases more)